...
const UserList = () => {
  ...
  return (
    <div style={{ width: window.innerWidth <= 768 ? '100%' : '40%' }}>
      <section id='listNavBar'>
        ...
        <div id='threeDot' ref={logoutRef} style={{ position: 'relative' }} onClick={() => setLogOutModel(prev => !prev)}>
          ...
          {logOutModel && (
            <ul className="deleteModel" onClick={() => { localStorage.removeItem('token'), localStorage.removeItem('LoginId'), navigate('/Auth') }}>
              <span>
                <IoIosLogOut
            ...
            >
              <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                ...
                <div style={{ marginTop: '12px' }}>
                  <h6 className="truncate">{user.user.name}</h6>
